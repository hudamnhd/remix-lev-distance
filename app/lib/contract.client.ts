...
export function contract() {
  const web3 = new Web3(
    new Web3.providers.HttpProvider(process.env.BLOCKHAIN_NETWORK),
  );
...
export function contractWithAddress(abi: any, deployed_address: any, network) {
  console.warn("DEBUGPRINT[6]: contract.client.ts:15: network=", network);
  const web3 = new Web3(new Web3.providers.HttpProvider(network));
  let contract = new web3.eth.Contract(abi, deployed_address);
  return contract;
}

export function contractEthWithAddress(
  abi,
  deployed_addres,
  network,
  JsonRpcProvider,
