...
import wallets from "~/constants/wallet.json";
...
export const loader = async () => {
  return json({ listPengajuan, wallets });
};

export default function Index() {
  const { listPengajuan, wallets } = useLoaderData<typeof loader>();

  const tasks = listPengajuan?.map((data, index) => {
  ...
  type Task = { address: string; [key: string]: any };
  type Wallet = { address: string; [key: string]: any };

  function compareAddresses(tasks: Task[], wallets: Wallet[]) {
    const sameAddresses: string[] = [];
    const differentAddresses: {
      inTasksOnly: string[];
      inWalletsOnly: string[];
    } = {
      inTasksOnly: [],
      inWalletsOnly: [],
    };
    ...
    // Check addresses in tasks
    tasks.forEach((task) => {
      const normalizedTaskAddress = task.address.trim().toLowerCase();
      if (walletAddresses.includes(normalizedTaskAddress)) {
        sameAddresses.push(normalizedTaskAddress);
      } else {
        differentAddresses.inTasksOnly.push(normalizedTaskAddress);
      }
    });

    // Check addresses in wallets
    const taskAddresses = tasks.map((task) =>
      task.address.trim().toLowerCase(),
    );
    wallets.forEach((wallet) => {
      const normalizedWalletAddress = wallet.address.trim().toLowerCase();
      if (!taskAddresses.includes(normalizedWalletAddress)) {
        differentAddresses.inWalletsOnly.push(normalizedWalletAddress);
      }
    });

    return { sameAddresses, differentAddresses };
  }
...
function compareAddresses(tasks: Task[], wallets: Wallet[]) {
  const result = tasks.map((task) => {
    ...
    return {
      taskAddress: task.address.trim(),
      isMatch,
      name: task.name,
    };
  });

  const unmatchedWallets = wallets.filter(
    (wallet) =>
      !tasks.some(
        (task) =>
          task.address.trim().toLowerCase() ===
  ...
  return { result, unmatchedWallets };
}

const AddressComparison: React.FC = () => {
  const { listPengajuan, wallets } = useLoaderData<typeof loader>();
  ...
  const { result, unmatchedWallets } = compareAddresses(tasks, wallets);
  const sortedNames = result
    .sort((a, b) =>
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Address Comparison</h1>
      <div className="grid grid-cols-2">
        {/* Tasks Column */}
        <div>
          <h2 className="text-lg font-semibold mb-2">Data DB</h2>
          <ul className="border border-gray-200">
            {sortedNames.map(({ name, taskAddress, isMatch }, index) => (
              <li
                key={index}
                className={`rounded ${isMatch ? "bg-green-100" : "bg-red-100"}`}
              >
                {name}-{taskAddress}
              </li>
            ))}
      ...
      <div className="mt-6">
        ...
        <p>
          ...
          <span className="text-green-600 font-bold">
            {result.filter((item) => item.isMatch).length}
          </span>
        </p>
        <p>
          Total Unmatched in Tasks:{" "}
          <span className="text-red-600 font-bold">
            {result.filter((item) => !item.isMatch).length}
          </span>
        </p>
        <p>
          Total Unmatched in Wallets:{" "}
          <span className="text-red-600 font-bold">
            {unmatchedWallets.length}
          </span>
        </p>
      </div>
    </div>
